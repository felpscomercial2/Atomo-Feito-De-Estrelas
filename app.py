...
def cliente_historico():
    """
    Retorna o histórico detalhado de faturamento (vendas e devoluções) 
    de um cliente específico, filtrando por cod_cliente ou nome.
    """
    try:
        cod_cliente = request.args.get('cod_cliente', '').strip()
        cliente_nome = request.args.get('cliente', '').strip()

        if not cod_cliente and not cliente_nome:
            return jsonify({'erro': 'Informe o cod_cliente ou o nome do cliente'}), 400
