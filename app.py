...
def shelflife_upload():
    ...
    # Registra todos os produtos deste upload no histórico permanente
    try:
        cursor2 = conn.cursor()
        cursor2.execute("""
        ...
        """)
        for p in produtos:
            if p.get('cod_produto'):
                cursor2.execute("""
                    INSERT INTO shelflife_produtos_vistos (cod_produto, unidade)
                    VALUES (%s, %s) ON CONFLICT DO NOTHING
                """, [p.get('cod_produto'), unidade])
        cursor2.close()
    except Exception:
        pass  # não bloqueia o upload se falhar
...
def shelflife_produtos_vistos():
    """Retorna todos os cod_produto que já apareceram no shelflife, para comparação de novos."""
    unidade = request.args.get('unidade', '')
    try:
        conn   = get_conn()
        cursor = conn.cursor()
        ...
        if unidade:
            ...
            cursor.execute("SELECT cod_produto, unidade FROM shelflife_produtos_vistos")
