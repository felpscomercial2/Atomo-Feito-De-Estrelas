    ...
    def _warm():
        try:
            ...
            filtros = {
                ...
                'vendedores': q("""
                SELECT nome FROM vendedores WHERE nome IS NOT NULL AND nome != ''
                UNION
                SELECT DISTINCT vendedor FROM faturamento WHERE vendedor IS NOT NULL AND vendedor != ''
                ORDER BY 1
            """),
            }
...
def filtros():
    ...
    resultado = {
        ...
        'marcas':     q("SELECT DISTINCT marca FROM faturamento WHERE marca IS NOT NULL ORDER BY marca"),
        'tipos':      q("SELECT DISTINCT tipo_operacao FROM faturamento WHERE tipo_operacao IS NOT NULL ORDER BY tipo_operacao"),
        # Vendedores: tabela vendedores (atual) UNION faturamento (histórico)
        # Garante que novos vendedores da carteira apareçam mesmo sem faturamento ainda
        'vendedores': q("""
            SELECT nome FROM vendedores WHERE nome IS NOT NULL AND nome != ''
            UNION
            SELECT DISTINCT vendedor FROM faturamento WHERE vendedor IS NOT NULL AND vendedor != ''
...
def shelflife_upload():
    ...
    for p in produtos:
        ...
        if dias <= 30:   status_gravidade = 'CRITICO'
        elif dias <= 60: status_gravidade = 'ATENCAO'
        else:            status_gravidade = 'OK'
...
def shelflife_listar():
    ...
    try:
        ...
        for row in resultado:
            ...
            # O status deve ser sempre a gravidade (CRITICO, ATENCAO, OK)
            # para que o usuário saiba a urgência, mesmo sendo SL.
            row['status'] = novo_status
            
            # Lógica de Tags solicitada pelo usuário:
            # 1. Produto Novo: Quando acabou de ser importado e ainda não houve interação (data_inc ou obs_log vazios)
            row['tag_novo'] = not row.get('data_inconsistencia') and not row.get('obs_logistica')
            
            # 2. Aumentou a Quantidade: Quando a quantidade logística (planilha) é maior que a conferida anteriormente
            qtd_log   = float(row.get('quantidade_log') or 0)
            qtd_atual = float(row.get('quantidade_atual') or 0)
            row['tag_qtd_maior'] = (qtd_atual > 0 and qtd_log > qtd_atual)

            status_map.append(row)

        # Filtros:
        if status == 'SL':
            # Se filtrar por SL, mostra apenas os que são is_sl
            status_map = [r for r in status_map if r.get('is_sl')]
        elif status in ['CRITICO', 'ATENCAO', 'OK']:
            # Se filtrar por gravidade, mostra inclusive os SLs que possuem essa gravidade
            status_map = [r for r in status_map if r.get('status') == status]
        elif status:
...
def shelflife_atualizar():
    ...
    cursor.execute(f"""
        UPDATE shelflife SET
            ...
            is_sl                = CASE WHEN %s IS NOT NULL THEN (%s AND %s NOT IN ('', '-', '—', 'nan', '0')) ELSE is_sl END,
            updated_at           = NOW()
        WHERE id = %s
    """, [
        ...
        *_data_inc_params,
        ...
        data.get('cod_sl'),  bool(data.get('cod_sl')), str(data.get('cod_sl')).strip(),
        id_prod
    ])
...
def pivot_clientes_novo():
    try:
        ...
        if produtos:
            ...
            vend_filter  = ''
            if vendedores:
                # Busca cod_vendedor atual na tabela vendedores pelo nome
                # Evita problema de vendedores que trocaram de código
                vend_ph  = ','.join(['%s'] * len(vendedores))
                cod_rows = consultar(
                    'SELECT DISTINCT cod_vendedor FROM vendedores WHERE nome IN (' + vend_ph + ') AND cod_vendedor IS NOT NULL',
                    list(vendedores)
                )
                vend_cods = [r['cod_vendedor'] for r in cod_rows]
                if not vend_cods:
                    # Fallback: busca pelo nome no faturamento
                    cod_rows = consultar(
                    ...
                    vend_cods = [r['cod_vendedor'] for r in cod_rows]
                if vend_cods:
                    cod_ph      = ','.join(['%s'] * len(vend_cods))
                    vend_filter = 'AND f.cod_vendedor IN (' + cod_ph + ')'
                    where_params += vend_cods
                else:
                    cursor.close(); conn.close()
                    return jsonify([])
            ...
            sql = (
                ...
                ' ' + periodo_filter +
                ' ' + cart_filter +
                ' GROUP BY c.cliente, c.cod_cliente, v.nome, f.ano, f.mes, c.endereco, c.bairro, c.cidade, c.uf'
