...
def pivot_clientes_novo():
    try:
        ...
        # Se nenhum periodo informado, limita ao ano atual para evitar scan completo
        if not periodo_conds and not periodos and not anos:
            from datetime import datetime as _dtnow
            ano_default = _dtnow.now().year
            periodo_conds.append('f.ano = %s')
            periodo_params.append(ano_default)
        ...
        if produtos:
            # COM produto: so clientes que compraram
            where_params = list(produtos)
            prod_ph      = ','.join(['%s'] * len(produtos))
            vend_filter  = ''
            if vendedores:
                vend_ph     = ','.join(['%s'] * len(vendedores))
                vend_filter = 'AND f.vendedor IN (' + vend_ph + ')'
                where_params += list(vendedores)

            sql = (
                'SELECT f.cliente, f.cod_cliente, f.vendedor, f.ano, f.mes,'
            ...
            # param order: Venda, Devolucao (CASE), produtos, vendedores, periodos
            cursor.execute(sql, ['Venda', 'Devolucao'] + where_params + periodo_params)

        else:
            # SEM produto: base carteira LEFT JOIN faturamento
            cart_filter  = ''
            cart_params  = []
            if vendedores:
                vend_ph  = ','.join(['%s'] * len(vendedores))
                cod_rows = consultar(
                    'SELECT DISTINCT cod_vendedor FROM faturamento WHERE vendedor IN (' + vend_ph + ') AND cod_vendedor IS NOT NULL AND cod_vendedor != %s',
                    vendedores + ['']
                )
                cods = [r['cod_vendedor'] for r in cod_rows]
                if cods:
                    cart_filter = 'WHERE c.cod_vendedor IN (' + ','.join(['%s']*len(cods)) + ')'
                    cart_params = cods
                else:
                    cursor.close(); conn.close()
                    return jsonify([])
            ...
            sql = (
                'SELECT c.cliente, c.cod_cliente, v.nome AS vendedor, f.ano, f.mes,'
                ' ROUND(COALESCE(SUM(CASE WHEN f.tipo_operacao = %s THEN f.valor_nf ELSE 0 END),0)::NUMERIC,2) AS faturamento,'
                ' ROUND(COALESCE(SUM(CASE WHEN f.tipo_operacao = %s THEN f.valor_nf ELSE 0 END),0)::NUMERIC,2) AS devolucoes'
                ' FROM carteira c'
                ' LEFT JOIN vendedores v ON v.cod_vendedor = c.cod_vendedor'
                ' LEFT JOIN faturamento f'
                ' ON f.cod_cliente = c.cod_cliente'
                ' AND f.tipo_operacao IN (%s, %s)'
                ' AND f.mes > 0'
                ' ' + periodo_filter +
                ' ' + cart_filter +
                ' GROUP BY c.cliente, c.cod_cliente, v.nome, f.ano, f.mes'
                ' ORDER BY c.cliente, f.ano, f.mes'
            )
            # param order: Venda, Devolucao (CASE), '' (vendedor != ''), Venda, Devolucao (IN), periodos, cart
            cursor.execute(sql, ['Venda', 'Devolucao', 'Venda', 'Devolucao'] + periodo_params + cart_params)
...
def valter_pesquisar():
    ...
    SITES = [
        {
            'nome': 'Alimentare Shop',
            'url': f'https://www.alimentareshop.com.br/?s={termo}',
            'base': 'https://www.alimentareshop.com.br',
            'seletor_item': ['article', '.product', '.woocommerce-loop-product', 'li.product'],
            'seletor_titulo': ['h2', 'h3', '.woocommerce-loop-product__title'],
            'seletor_desc': ['.short-description', '.product-short-description', 'p'],
        },
        {
            'nome': 'Alimentare',
            'url': f'https://www.alimentare.com.br/?s={termo}',
            'base': 'https://www.alimentare.com.br',
            'seletor_item': ['article', '.post', '.product'],
            'seletor_titulo': ['h2', 'h1', '.entry-title'],
            'seletor_desc': ['.entry-content', '.entry-summary', 'p'],
        },
        {
            'nome': 'Esdel',
            'url': f'https://www.esdel.com.br/?s={termo}',
            'base': 'https://www.esdel.com.br',
            'seletor_item': ['article', '.product', '.post'],
            'seletor_titulo': ['h2', 'h1', '.entry-title'],
            'seletor_desc': ['.entry-content', '.entry-summary', 'p'],
        },
        {
            'nome': 'Reforpan',
            'url': f'https://www.reforpan.com.br/?s={termo}',
            'base': 'https://www.reforpan.com.br',
            'seletor_item': ['article', '.product', '.post'],
            'seletor_titulo': ['h2', 'h1', '.entry-title'],
            'seletor_desc': ['.entry-content', '.entry-summary', 'p'],
        },
    ...
    for site in SITES:
        try:
            resp = _req.get(site['url'], headers=HEADERS, timeout=8)
            if resp.status_code != 200:
                continue
            soup = _BS(resp.text, 'html.parser')

            # Tenta encontrar itens de produto/post
            itens = []
            for sel in site['seletor_item']:
                itens = soup.select(sel)
                if itens:
                    break

            if not itens:
                # Fallback: busca qualquer menção ao produto no body
                texto_pagina = soup.get_text(separator=' ', strip=True)
                if produto.lower() in texto_pagina.lower():
                    # Extrai parágrafo relevante
                    paragrafos = soup.find_all('p')
                    trechos = []
                    for p in paragrafos:
                        txt = p.get_text(strip=True)
                        if produto.lower() in txt.lower() and len(txt) > 30:
                            trechos.append(txt[:300])
                        if len(trechos) >= 2:
                            break
                    if trechos:
                        resultados.append({
            ...
            for item in itens[:3]:
                # Título
                titulo = ''
                for sel in site['seletor_titulo']:
                ...
                # Descrição
                desc = ''
                for sel in site['seletor_desc']:
                    el = item.select_one(sel)
                    if el:
                        desc = el.get_text(separator=' ', strip=True)[:400]
                        break
                ...
                if link_el and link_el.get('href'):
                    href = link_el['href']
                    link = href if href.startswith('http') else site['base'] + href

                if titulo or desc:
                    resultados.append({
                        'fonte': site['nome'],
                        'url': link or site['url'],
                        'titulo': titulo or produto,
                        'descricao': desc,
                    })

            if resultados:
                break  # Encontrou no primeiro site, não precisa continuar

        except Exception as e:
            continue  # Site inacessível, tenta próximo

    return jsonify({
        'produto': produto,
        'resultados': resultados,
        'total': len(resultados),
        'fontes_consultadas': [s['nome'] for s in SITES],
    })



# ============================================================
#  VALTER — PROXY CHAT (evita CORS do browser → Anthropic)
# ============================================================
...
def valter_chat():
    ...
    try:
        ...
        if not api_key:
            return jsonify({'erro': 'GROQ_API_KEY nao configurada no Railway'}), 500

        system_prompt = data.get('system', '')
        messages      = data.get('messages', [])

        # Groq usa formato OpenAI: system vai como primeira mensagem
        groq_messages = []
        if system_prompt:
        ...
        # Converte resposta Groq para formato Anthropic (que o frontend espera)
        texto = ''
        if 'choices' in groq_data and groq_data['choices']:
            texto = groq_data['choices'][0].get('message', {}).get('content', '')
...
# ============================================================
#  VALTER — HISTÓRICO DE PRODUTOS VISTOS NO SHELFLIFE
# ============================================================
@app.route('/api/shelflife/produtos-vistos')
def shelflife_produtos_vistos():
    unidade = request.args.get('unidade', '')
    try:
        conn   = get_conn()
        cursor = conn.cursor()
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS shelflife_produtos_vistos (
                cod_produto TEXT NOT NULL,
                unidade     TEXT NOT NULL,
                primeiro_upload TIMESTAMP DEFAULT NOW(),
                PRIMARY KEY (cod_produto, unidade)
            )
        """)
        conn.commit()
        if unidade:
            cursor.execute(
                "SELECT cod_produto FROM shelflife_produtos_vistos WHERE unidade = %s",
                [unidade]
            )
        else:
            cursor.execute("SELECT cod_produto FROM shelflife_produtos_vistos")
        rows = cursor.fetchall()
        cursor.close(); conn.close()
...
# ============================================================
#  VALTER — CONTEXTO GERAL DO SISTEMA
#  Busca resumo de todas as áreas para o Valter responder
#  perguntas de qualquer página do Átomo
# ============================================================
@app.route('/api/valter/contexto')
def valter_contexto():
    try:
        from datetime import datetime as _dtn
        ano_atual = _dtn.now().year
        mes_atual = _dtn.now().month
        conn   = get_conn()
        cursor = conn.cursor()
        ...
        cursor.execute("""
            SELECT
                ROUND(SUM(CASE WHEN tipo_operacao='Venda' THEN valor_nf ELSE 0 END)::NUMERIC,2) AS fat_mes,
                ROUND(SUM(CASE WHEN tipo_operacao='Devolucao' THEN valor_nf ELSE 0 END)::NUMERIC,2) AS dev_mes
            FROM faturamento WHERE ano = %s AND mes = %s
        """, [ano_atual, mes_atual])
        kpis_mes = cursor.fetchone()

        # Top 5 vendedores ano atual
        cursor.execute("""
            SELECT vendedor, ROUND(SUM(valor_nf)::NUMERIC,2) AS fat
            FROM faturamento WHERE ano = %s AND tipo_operacao='Venda' AND vendedor IS NOT NULL
            GROUP BY vendedor ORDER BY fat DESC LIMIT 5
        """, [ano_atual])
        top_vendedores = cursor.fetchall()

        # Top 5 marcas ano atual
        cursor.execute("""
            SELECT marca, ROUND(SUM(valor_nf)::NUMERIC,2) AS fat
            FROM faturamento WHERE ano = %s AND tipo_operacao='Venda' AND marca IS NOT NULL
            GROUP BY marca ORDER BY fat DESC LIMIT 5
        """, [ano_atual])
        top_marcas = cursor.fetchall()

        # Top 5 produtos ano atual
        cursor.execute("""
            SELECT produto, ROUND(SUM(valor_nf)::NUMERIC,2) AS fat
            FROM faturamento WHERE ano = %s AND tipo_operacao='Venda' AND produto IS NOT NULL
            GROUP BY produto ORDER BY fat DESC LIMIT 5
        """, [ano_atual])
        top_produtos = cursor.fetchall()

        # ShelfLife — resumo críticos
        cursor.execute("""
            SELECT
                COUNT(*) AS total,
                SUM(CASE WHEN status_logistica='CRITICO' OR dias_vencimento <= 30 THEN 1 ELSE 0 END) AS criticos,
                SUM(CASE WHEN is_sl = TRUE THEN 1 ELSE 0 END) AS em_sl
            FROM shelflife
            WHERE semana = (SELECT MAX(semana) FROM shelflife)
        """)
        sl_resumo = cursor.fetchone()

        # Carteira — resumo
        cursor.execute("SELECT COUNT(*) FROM carteira")
        total_carteira = cursor.fetchone()[0]
        ...
        contexto = f"""SISTEMA ÁTOMO — RESUMO GERAL ({ano_atual})
==============================================
FATURAMENTO ANO: R$ {kpis[0]:,.2f} | DEVOLUÇÕES: R$ {kpis[1]:,.2f}
FATURAMENTO {meses_nome.get(mes_atual,'')}/{ano_atual}: R$ {kpis_mes[0]:,.2f} | DEV: R$ {kpis_mes[1]:,.2f}
CLIENTES ATIVOS: {kpis[2]:,} | VENDEDORES: {kpis[3]}
CARTEIRA TOTAL: {total_carteira:,} clientes

TOP 5 VENDEDORES ({ano_atual}):
{chr(10).join(f"  {i+1}. {r[0]} — R$ {r[1]:,.2f}" for i,r in enumerate(top_vendedores))}

TOP 5 MARCAS ({ano_atual}):
{chr(10).join(f"  {i+1}. {r[0]} — R$ {r[1]:,.2f}" for i,r in enumerate(top_marcas))}

TOP 5 PRODUTOS ({ano_atual}):
{chr(10).join(f"  {i+1}. {r[0]} — R$ {r[1]:,.2f}" for i,r in enumerate(top_produtos))}

SHELF LIFE (semana atual):
  Total: {sl_resumo[0]} produtos | Críticos: {sl_resumo[1]} | Em ação SL: {sl_resumo[2]}
"""
        return jsonify({'contexto': contexto, 'ok': True})
    except Exception as e:
        import traceback
        return jsonify({'erro': str(e), 'trace': traceback.format_exc()}), 500
...
#  Verifica situações urgentes para notificar o usuário
# ============================================================
...
def valter_alertas():
    try:
        ...
        # Produtos SL vencendo em 7 dias
        cursor.execute("""
            SELECT COUNT(*) FROM shelflife
            WHERE semana = (SELECT MAX(semana) FROM shelflife)
            AND dias_vencimento <= 7 AND dias_vencimento >= 0
            AND is_sl = TRUE
        """)
        sl_urgente = cursor.fetchone()[0]
        if sl_urgente > 0:
            alertas.append({
                'icone': '🚨',
                'texto': f'{sl_urgente} produto{"s" if sl_urgente>1 else ""} SL vencem em 7 dias',
                'pergunta': f'Liste os {sl_urgente} produtos SL que vencem em 7 dias e sugira ações urgentes.',
            })

        # Produtos críticos sem ação
        cursor.execute("""
            SELECT COUNT(*) FROM shelflife
            WHERE semana = (SELECT MAX(semana) FROM shelflife)
            AND (status_logistica = 'CRITICO' OR dias_vencimento <= 30)
            AND (acao IS NULL OR acao = '' OR acao = 'Lembrete')
        """)
        criticos_sem_acao = cursor.fetchone()[0]
        if criticos_sem_acao > 0:
            alertas.append({
                'icone': '⚠️',
                'texto': f'{criticos_sem_acao} produto{"s" if criticos_sem_acao>1 else ""} crítico{"s" if criticos_sem_acao>1 else ""} sem ação definida',
                'pergunta': f'Quais são os {criticos_sem_acao} produtos críticos sem ação definida? Sugira o que fazer.',
            })
