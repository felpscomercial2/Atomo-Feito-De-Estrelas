    ...
    try:
        ...
        if produtos:
            ...
            sql = (
                ...
                ' MAX(f.cidade) AS cidade, MAX(f.uf) AS uf, MAX(f.bairro) AS bairro,'
...
# ============================================================
# COMPRAS DIÁRIAS — ENDPOINTS COMPLETOS
# Cole este bloco no final do seu app.py (substitua a seção
# "COMPRAS DIÁRIAS" existente, se houver).
#
# Requisitos: psycopg2, openpyxl
...
def compras_upload():
    ...
    try:
        ...
        nome_arquivo = data.get('nomeArquivo', 'Compras.xlsx')
        upload_id_existente = data.get('uploadIdExistente')
        if not items:
        ...
        # Detecta a BASE (PR ou RS) pelo nome do arquivo; se não achar, usa o
        # campo BASE do primeiro item da planilha
        nome_upper = nome_arquivo.upper()
        if 'RS' in nome_upper and 'PR' not in nome_upper:
            base_detectada = 'RS'
        elif 'PR' in nome_upper or 'CGS' in nome_upper:
            base_detectada = 'PR'
        else:
            base_detectada = str(items[0].get('base', '')).strip().upper() or '?'

        if upload_id_existente:
            # Lote adicional de um upload já iniciado
            novo_upload_id = upload_id_existente
            cursor.execute(
                "UPDATE purchases_uploads SET totalitems = totalitems + %s WHERE id = %s",
                [len(items), novo_upload_id]
            )
        else:
            # Busca o primeiro usuário disponível no banco
            ...
            cursor.execute("""
                INSERT INTO purchases_uploads (userid, filename, totalitems, base)
                VALUES (%s, %s, %s, %s) RETURNING id
            """, [user_id, nome_arquivo, len(items), base_detectada])
        ...
        for item in items:
            cursor.execute("""
                INSERT INTO purchase_items
                    (uploadid, codigoproduto, descricaoproduto, quantidademediavenda, marca,
                     carimbodatahora, nomevendedor, previsaochegada)
            ...
            """, [
                ...
                str(item.get('nomeVendedor', '')),
                str(item.get('previsaoChegada', '')),
            ])

        # KPIs comparativos — REPETIDO = produto (por nome) já apareceu em
        # QUALQUER planilha anterior da mesma base. Só calcula no primeiro lote.
        existiam_na_anterior = 0
        novos_produtos = len(items)
        if not upload_id_existente:
            nomes_atuais = [str(i.get('nomeProduto', '')).strip().lower()
                             for i in items if i.get('nomeProduto')]
            cursor.execute("""
                SELECT DISTINCT lower(trim(pi.descricaoproduto))
                FROM purchase_items pi
                JOIN purchases_uploads pu ON pu.id = pi.uploadid
                WHERE pu.base = %s AND pu.id != %s
            """, [base_detectada, novo_upload_id])
            nomes_historico = {r[0] for r in cursor.fetchall()}
            existiam_na_anterior = sum(1 for n in nomes_atuais if n in nomes_historico)
            novos_produtos = len(nomes_atuais) - existiam_na_anterior
...
def compras_snapshots():
    ...
    try:
        ...
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cursor.execute("""
            SELECT id, filename, totalitems, base,
...
def compras_listar():
    ...
    try:
        ...
        cursor.execute("""
            SELECT carimbodatahora, base, nomevendedor, codigoproduto,
                   descricaoproduto, quantidademediavenda, marca, previsaochegada
            FROM purchase_items
            WHERE uploadid = %s
            ORDER BY id
        """, [upload_id])
        ...
        # Descobre a base deste upload
        cursor.execute("SELECT base FROM purchases_uploads WHERE id = %s", [upload_id])
        row_base = cursor.fetchone()
        base_atual = row_base['base'] if row_base else None

        # "Anteriores" = nomes de produto que já apareceram em QUALQUER
        # planilha anterior da mesma base (histórico completo)
        nomes_anteriores = []
        if base_atual:
            cursor.execute("""
                SELECT DISTINCT lower(trim(pi.descricaoproduto)) AS nome
                FROM purchase_items pi
                JOIN purchases_uploads pu ON pu.id = pi.uploadid
                WHERE pu.base = %s AND pu.id != %s
            """, [base_atual, upload_id])
            nomes_anteriores = [r['nome'] for r in cursor.fetchall()]

        cursor.close(); conn.close()
        return jsonify({'items': items, 'codigosAnteriores': nomes_anteriores})
    except Exception as e:
...
def compras_exportar():
    ...
    try:
        ...
        ws = wb.active
        ws.title = 'Compras'
        ws.append(['Data', 'Base', 'Vendedor', 'Código', 'Produto', 'Quantidade', 'Fabricante', 'Previsão Chegada'])
        for r in rows:
            ws.append([r['carimbodatahora'], r['base'], r['nomevendedor'], r['codigoproduto'],
                       r['descricaoproduto'], r['quantidademediavenda'], r['marca'], r['previsaochegada']])
