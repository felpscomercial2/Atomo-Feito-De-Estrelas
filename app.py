...
app = Flask(__name__)
CORS(app)  # base, será sobrescrito pelo after_request abaixo

ORIGENS_PERMITIDAS = [
    'https://horus-snowy.vercel.app',
    'https://horus-bmcj.onrender.com',
]

@app.after_request
def aplicar_cors(response):
    origin = request.headers.get('Origin', '')
    # Permite vercel.app, localhost e render.com
    if (origin.endswith('.vercel.app') or
        origin.endswith('.onrender.com') or
        origin.startswith('http://localhost') or
        origin.startswith('http://127.0.0.1') or
        origin in ORIGENS_PERMITIDAS):
...
def dashboard():
    """
    Retorna KPIs + faturamento mensal + por unidade em uma única chamada.
    Substitui 3 requests separados na visão geral.
    """
    ...
    cursor.execute(f"""
        SELECT
            ...
            COUNT(DISTINCT CASE WHEN tipo_operacao = 'Venda' THEN cliente END) AS total_clientes,
            COUNT(CASE WHEN tipo_operacao = 'Venda' THEN 1 END) AS qtd_vendas,
            ROUND(CAST(AVG(CASE WHEN tipo_operacao = 'Venda' AND margem IS NOT NULL AND margem != 0 THEN margem END) AS NUMERIC),2) AS margem_media
        FROM faturamento {where}
    """, params)
    kpis = _serializar_row(dict(cursor.fetchone() or {}))
    ...
    """, params)
    mensal = [_serializar_row(dict(r)) for r in cursor.fetchall()]
    ...
    cursor.execute(f"""
        SELECT unidade,
            ROUND(CAST(SUM(CASE WHEN tipo_operacao = 'Venda'     THEN valor_nf ELSE 0 END) AS NUMERIC),2) AS faturamento,
            ROUND(CAST(SUM(CASE WHEN tipo_operacao = 'Devolucao' THEN valor_nf ELSE 0 END) AS NUMERIC),2) AS devolucoes,
            COUNT(DISTINCT CASE WHEN tipo_operacao = 'Venda' THEN cliente END) AS clientes
        FROM faturamento {where} {and_or} unidade IS NOT NULL
        GROUP BY unidade ORDER BY faturamento DESC
    """, params)
    por_unidade = [_serializar_row(dict(r)) for r in cursor.fetchall()]
    ...
    """, params)
    top_vendedores = [_serializar_row(dict(r)) for r in cursor.fetchall()]
    ...
    """, params)
    top_marcas = [_serializar_row(dict(r)) for r in cursor.fetchall()]
...
def shelflife_upload():
    ...
    for p in produtos:
        ...
        is_sl = bool(re.match(r'^sl[.\s]', nome, re.IGNORECASE))
...
def shelflife_listar():
    ...
    try:
        ...
        if status == 'SL':
            status_map = [r for r in status_map if r.get('is_sl') or re.match(r'^sl[.\s]', str(r.get('produto','')), re.IGNORECASE)]
        elif status:
...
def shelflife_exportar():
    ...
    col_widths = [10, 12, 12, 36, 18, 8, 14, 12, 12, 13, 14, 15, 14, 19, 26, 26, 20, 20]
    for i, w in enumerate(col_widths, 1):
        ws.column_dimensions[get_column_letter(i)].width = w

    NUM_COLS    = {7, 8, 9, 11, 12, 13}  # Qtde Log, Qtde Ant, Qtde Atual, Dias, V3M, VMes
    CENTER_COLS = {1, 2, 3, 6, 10, 11, 14, 17}
...
def pivot_clientes_novo():
    try:
        ...
        if produtos:
            ...
            sql = (
                'SELECT c.cliente, c.cod_cliente, v.vendedor, f.ano, f.mes,'
                ...
                ' FROM carteira c'
                ' LEFT JOIN (SELECT DISTINCT cod_vendedor, vendedor FROM faturamento WHERE vendedor IS NOT NULL AND vendedor != %s) v'
