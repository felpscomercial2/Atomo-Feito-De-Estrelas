...
def pivot_clientes_novo():
    ...
    try:
        ...
        vend_params = []
        if cods:
            # Total tem que bater 100% com a planilha:
            # entra TODA nota emitida pelo vendedor (fat_cod_vendedor), mesmo que o
            # cliente esteja na carteira de outro vendedor, + os clientes da carteira dele.
            vend_filter = (' AND (m.cart_cod_vendedor::TEXT = ANY(%s::TEXT[])'
                           ' OR m.fat_cod_vendedor::TEXT = ANY(%s::TEXT[]))')
        ...
        if produtos:
            ...
            sql = (
                ...
                ' FROM m'
                ' LEFT JOIN vendedores v ON v.cod_vendedor = COALESCE(m.cart_cod_vendedor, m.fat_cod_vendedor)'
    ...
    try:
        ...
        resultado = consultar(f"""
            SELECT
                ...
                ano,
                mes,
                ROUND(CAST(SUM(valor_nf) AS NUMERIC), 2) AS valor_nf
            FROM faturamento
            {where_str}
            GROUP BY cod_cliente, cod_produto, produto, ano, mes
        """, params)
