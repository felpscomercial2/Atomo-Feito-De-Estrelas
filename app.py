...
def valter_pesquisar():
    ...
    SITES_PROPRIOS = [
        'https://www.alimentareshop.com.br/?s=' + termo,
        'https://www.alimentare.com.br/?s=' + termo,
        'https://www.esdel.com.br/?s=' + termo,
        'https://www.reforpan.com.br/?s=' + termo,
    ]
    NOMES_SITES = ['Alimentare Shop', 'Alimentare', 'Esdel', 'Reforpan']

    resultados = []

    def extrair_texto(soup, produto):
        """Extrai trechos relevantes de qualquer página"""
        texto = []
        # Tenta seletores de produto WooCommerce / e-commerce
        for sel in ['article', '.product', 'li.product', '.woocommerce-loop-product',
                    '.entry-content', '.product-description', '.product-short-description',
        ...
        # Fallback: qualquer parágrafo relevante
        if not texto:
            for p in soup.find_all(['p', 'li', 'td', 'span']):
                t = p.get_text(strip=True)
                palavras_busca = produto.lower().split()
                if len(t) > 30 and any(w in t.lower() for w in palavras_busca if len(w) > 3):
                    texto.append(t[:400])
                if len(texto) >= 3:
                    break
    ...
    # 1. Tenta os sites próprios da Alimentare
    for i, url in enumerate(SITES_PROPRIOS):
        try:
            ...
            palavras = [w for w in produto.lower().split() if len(w) > 3]
            relevante = sum(1 for w in palavras if w in page_text) >= max(1, len(palavras) // 2)

            if not relevante:
                continue

            desc = extrair_texto(soup, produto)
            ...
            for a in soup.find_all('a', href=True):
                ...
                if any(w in txt for w in palavras if len(w) > 3):
            ...
            if desc:
                ...
                break  # Encontrou no site próprio, para aqui

        except Exception:
            continue

    # 2. Se não encontrou nos sites próprios, busca no Google com site: restrito
    if not resultados:
        sites_query = ' OR '.join([
            'site:alimentareshop.com.br',
            'site:alimentare.com.br',
            'site:esdel.com.br',
            'site:reforpan.com.br',
        ])
        google_url = f'https://www.google.com/search?q={termo}+({urllib.parse.quote(sites_query)})&hl=pt-BR&num=5'
        try:
            resp = _req.get(google_url, headers=HEADERS, timeout=10)
            soup = _BS(resp.text, 'html.parser')
            # Extrai snippets dos resultados
            for div in soup.select('.g, .tF2Cxc, [data-sokoban-container]')[:4]:
                titulo_el = div.select_one('h3')
                desc_el   = div.select_one('.VwiC3b, .s3v9rd, .st, span[style]')
                link_el   = div.select_one('a[href]')
                titulo = titulo_el.get_text(strip=True) if titulo_el else produto
                desc   = desc_el.get_text(strip=True) if desc_el else ''
                link   = link_el.get('href', '') if link_el else ''
                if link.startswith('/url?q='):
                    link = link[7:link.find('&')] if '&' in link else link[7:]
                if titulo or desc:
                    resultados.append({
                        'fonte': 'Google',
                        'url': link,
    ...
    # 3. Se ainda não encontrou, busca no Google sem restrição de site
    if not resultados:
        google_url = f'https://www.google.com/search?q={termo}+produto+alimenticio+codigo+barras+ingredientes&hl=pt-BR&num=5'
        try:
            resp = _req.get(google_url, headers=HEADERS, timeout=10)
            soup = _BS(resp.text, 'html.parser')
            for div in soup.select('.g, .tF2Cxc')[:4]:
                titulo_el = div.select_one('h3')
                desc_el   = div.select_one('.VwiC3b, .s3v9rd, .st')
                link_el   = div.select_one('a[href]')
                titulo = titulo_el.get_text(strip=True) if titulo_el else produto
                desc   = desc_el.get_text(strip=True) if desc_el else ''
                link   = link_el.get('href', '') if link_el else ''
    ...
    return jsonify({
        ...
        'fontes_consultadas': NOMES_SITES + (['Google'] if not resultados else []),
    })
