    ...
    def _warm():
        try:
            ...
            filtros = {
                ...
                'vendedores': (lambda: (
            lambda vends_fat, vends_tab:
                sorted(list(set([v['vendedor'] for v in vends_fat] + [v['nome'] for v in vends_tab])))
        )(
            q("SELECT DISTINCT vendedor FROM faturamento WHERE vendedor IS NOT NULL AND vendedor != '' ORDER BY vendedor"),
            q("SELECT nome FROM vendedores ORDER BY nome") if True else []
        ))(),
            }
...
def pivot_clientes_novo():
    try:
        ...
        if produtos:
            ...
            if vendedores:
                vend_ph  = ','.join(['%s'] * len(vendedores))
                # Busca cod_vendedor tanto pelo nome no faturamento quanto na tabela vendedores
                cod_rows = consultar(
                ...
                # Também busca na tabela vendedores (nome oficial da planilha)
                try:
                    vend_rows = consultar(
                        'SELECT DISTINCT cod_vendedor FROM vendedores WHERE nome IN (' + vend_ph + ')',
                        vendedores
                    )
                    cods += [r['cod_vendedor'] for r in vend_rows if r['cod_vendedor'] not in cods]
                except Exception:
                    pass
                if cods:
