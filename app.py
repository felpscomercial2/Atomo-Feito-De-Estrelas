...
def pivot_clientes_novo():
    try:
        ...
        if produtos:
            ...
            vend_cods    = []
            if vendedores:
                # Busca cod_vendedor atual na tabela vendedores (não pelo histórico do faturamento)
            ...
            cart_params  = []
            if vendedores:
                # Busca cod_vendedor na tabela vendedores (nome atual)
                # Evita o problema de vendedores que trocaram de código:
                # o faturamento histórico tem nomes antigos, mas vendedores tem o atual
                vend_ph  = ','.join(['%s'] * len(vendedores))
                cod_rows = consultar(
                    'SELECT DISTINCT cod_vendedor FROM vendedores WHERE nome IN (' + vend_ph + ') AND cod_vendedor IS NOT NULL',
                    vendedores
                ...
                if not cods:
                    # Fallback: tenta pelo nome mais recente no faturamento
                    # (para vendedores que ainda não estão na tabela vendedores)
                    cod_rows = consultar(
                        'SELECT DISTINCT cod_vendedor FROM faturamento WHERE vendedor IN (' + vend_ph + ') AND cod_vendedor IS NOT NULL AND cod_vendedor != %s',
                        vendedores + ['']
                    )
