...
def pivot_clientes_novo():
    """
    Cruza FATURAMENTO x CARTEIRA usando chave HÍBRIDA (código + CNPJ).

    Prioridade de match (o mesmo cliente pode ter cadastro em várias bases,
    então nem o código nem o CNPJ isolados são confiáveis):
      1) código do cliente E CNPJ iguais   -> match perfeito
      2) código do cliente igual            -> cadastro da mesma base
      3) CNPJ igual                         -> mesmo cliente em outra base
      Em empates, dá preferência à carteira do(s) vendedor(es) filtrado(s).
    Vendas sem nenhum match continuam aparecendo (sem_carteira = true),
    para que o total bata sempre com a planilha.
    """
    try:
        ...
        # ---------- descobre as colunas de CNPJ (nomes variam por importação) ----------
        ...
        def nz(expr):
            # normaliza CNPJ/CPF: só dígitos, sem zeros à esquerda
        ...
        # ---------- códigos dos vendedores filtrados ----------
        cods = []
        if vendedores:
            ...
            cods = [str(r['cod_vendedor']) for r in cod_rows if r.get('cod_vendedor')]

        # ---------- CTEs de match híbrido ----------
        base_cte = (
            'WITH cart AS ('
            '  SELECT c.cod_cliente, c.cliente, c.endereco, c.cidade, c.uf, c.cod_vendedor,'
            '         ' + c_cnpj_sql + ' AS cnpj_n'
            '  FROM carteira c'
            '), fat AS ('
            ...
            '  ' + periodo_filter +
            '), m AS ('
            '  SELECT fat.*, mc.cod_vendedor AS cart_cod_vendedor, mc.cliente AS cart_cliente,'
            '         mc.cod_cliente AS cart_cod_cliente, mc.endereco AS cart_endereco'
            '  FROM fat'
            '  LEFT JOIN LATERAL ('
            '    SELECT c.* FROM cart c'
            '    WHERE c.cod_cliente = fat.cod_cliente'
            '       OR (c.cnpj_n <> %s AND c.cnpj_n = fat.cnpj_n)'
            '    ORDER BY (c.cod_cliente = fat.cod_cliente AND c.cnpj_n = fat.cnpj_n) DESC,'
            '             (c.cod_cliente = fat.cod_cliente) DESC,'
            '             (c.cod_vendedor = ANY(%s)) DESC,'
            '             c.cod_cliente'
            '    LIMIT 1'
            '  ) mc ON TRUE'
            ')'
        )
        base_params = ['Venda', 'Devolucao'] + periodo_params + ['', cods]
        ...
        vend_params = []
        if cods:
            vend_filter = ' AND m.cart_cod_vendedor = ANY(%s)'
            vend_params = [cods]
        elif vendedores:
        ...
        if produtos:
            ...
            sql = (
                ...
                + vend_filter +
                ' GROUP BY COALESCE(m.cart_cliente, m.cliente), m.cod_cliente, v.nome, m.ano, m.mes'
                ' ORDER BY 1, m.ano, m.mes'
            )
            cursor.execute(sql, base_params + ['Venda', 'Devolucao'] + list(produtos) + vend_params)

        else:
            # SEM produto: vendas casadas + clientes da carteira que não compraram
            cart_zero = ''
            zero_params = []
            if cods:
                cart_zero = (
                    ' UNION ALL'
                    ' SELECT c.cliente, c.cod_cliente, v.nome AS vendedor, NULL::INT AS ano, NULL::INT AS mes,'
                    ' c.cidade, c.uf, NULL AS bairro, c.endereco, FALSE AS sem_carteira,'
                    ' 0::NUMERIC AS faturamento, 0::NUMERIC AS devolucoes'
                    ' FROM carteira c'
                    ' LEFT JOIN vendedores v ON v.cod_vendedor = c.cod_vendedor'
                    ' WHERE c.cod_vendedor = ANY(%s)'
                    ' AND NOT EXISTS (SELECT 1 FROM m WHERE m.cart_cod_cliente = c.cod_cliente'
                    '                 AND m.cart_cod_vendedor = c.cod_vendedor)'
                )
                zero_params = [cods]
