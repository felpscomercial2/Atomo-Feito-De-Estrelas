    ...
    try:
        ...
        if produtos:
            ...
            if vendedores:
                ...
                # Filtra pelo nome do vendedor cadastrado na CARTEIRA (v.nome),
                # e não pelo vendedor gravado na nota fiscal antiga (f.vendedor).
                vend_filter = 'AND v.nome IN (' + vend_ph + ')'
                where_params += list(vendedores)

            sql = (
                # Vendedor agora vem da CARTEIRA (via cod_vendedor -> tabela vendedores),
                # que é a fonte correta/atualizada. Antes vinha de f.vendedor (faturamento),
                # que reflete o vendedor da epoca da nota, podendo estar desatualizado.
                'SELECT f.cliente, f.cod_cliente, v.nome AS vendedor, f.ano, f.mes,'
                ' MAX(f.cidade) AS cidade, MAX(f.uf) AS uf, MAX(f.bairro) AS bairro,'
                ' MAX(c.endereco) AS endereco,'
                ...
                ' FROM faturamento f'
                ' LEFT JOIN carteira c ON c.cod_cliente = f.cod_cliente'
                ' LEFT JOIN vendedores v ON v.cod_vendedor = c.cod_vendedor'
                ' WHERE f.produto IN (' + prod_ph + ')'
                ' AND f.mes > 0'
                ' ' + vend_filter +
                ' ' + periodo_filter +
                ' GROUP BY f.cliente, f.cod_cliente, v.nome, f.ano, f.mes'
                ' ORDER BY f.cliente, f.ano, f.mes'
            )
            cursor.execute(sql, ['Venda', 'Devolucao'] + where_params + periodo_params)

        else:
            # SEM produto: base carteira LEFT JOIN faturamento (traz todos da carteira do vendedor)
            cart_filter  = ''
            cart_params  = []
            if vendedores:
                vend_ph  = ','.join(['%s'] * len(vendedores))
                # Busca os cod_vendedor correspondentes aos nomes dos vendedores na tabela 'vendedores' ou 'faturamento'
                cod_rows = consultar(
                    'SELECT DISTINCT cod_vendedor FROM vendedores WHERE nome IN (' + vend_ph + ') '
                    'UNION '
                    'SELECT DISTINCT cod_vendedor FROM faturamento WHERE vendedor IN (' + vend_ph + ') AND cod_vendedor IS NOT NULL',
                    vendedores + vendedores
                )
                cods = [r['cod_vendedor'] for r in cod_rows if r.get('cod_vendedor')]
                if cods:
                    cart_filter = 'WHERE c.cod_vendedor IN (' + ','.join(['%s']*len(cods)) + ')'
                    cart_params = cods
                else:
                    # Fallback caso filtre direto pelo nome na tabela carteira se houver coluna correspondente
                    cart_filter = 'WHERE v.nome IN (' + vend_ph + ')'
                    cart_params = vendedores

            sql = (
                'SELECT c.cliente, c.cod_cliente, c.endereco, v.nome AS vendedor, f.ano, f.mes,'
                ' MAX(COALESCE(f.cidade, c.cidade)) AS cidade, MAX(COALESCE(f.uf, c.uf)) AS uf, MAX(f.bairro) AS bairro,'
                ...
                ' ' + periodo_filter +
                ' ' + cart_filter +
                ' GROUP BY c.cliente, c.cod_cliente, c.endereco, v.nome, f.ano, f.mes'
