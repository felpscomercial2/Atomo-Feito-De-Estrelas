...
def compras_upload():
    ...
    try:
        ...
        nome_arquivo = data.get('nomeArquivo', 'Compras_Diarias.xlsx')
        if not items:
        ...
        # Último upload para comparação de novos/repetidos
        ...
        # Detecta base pelo nome do arquivo (CGS ou RS)
        nome_upper = nome_arquivo.upper()
        if 'CGS' in nome_upper:
            base = 'CGS'
        elif 'RS' in nome_upper:
            base = 'RS'
        else:
            base = items[0].get('base', '') if items else ''

        cursor.execute("""
            INSERT INTO purchases_uploads (userid, filename, totalitems, base)
            VALUES (%s, %s, %s, %s) RETURNING id
        """, [user_id, nome_arquivo, len(items), base])
        novo_upload_id = cursor.fetchone()[0]

        for item in items:
            cursor.execute("""
                INSERT INTO purchase_items
                    (uploadid, carimbodatahora, base, codigoproduto,
                     descricaoproduto, quantidademediavenda, marca)
                VALUES (%s, %s, %s, %s, %s, %s, %s)
            """, [
                novo_upload_id,
                str(item.get('dataHora', '')),
                str(item.get('base', base)),
...
def compras_snapshots():
    """Lista todas as planilhas enviadas agrupadas por data."""
    try:
        conn   = get_conn()
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cursor.execute("""
            SELECT id, filename, totalitems, base,
                   to_char(uploaddate, 'DD/MM/YYYY') AS data,
...
def compras_listar():
    """Retorna os itens de um upload + códigos anteriores para marcar NOVO."""
    try:
        ...
        cursor.execute("""
            SELECT carimbodatahora, base, codigoproduto,
                   descricaoproduto, quantidademediavenda, marca
            FROM purchase_items
            WHERE uploadid = %s
            ORDER BY id
...
def compras_exportar():
    ...
    try:
        ...
        ws = wb.active
        ws.title = 'Compras'
        ws.append(['Data', 'Base', 'Código', 'Produto', 'Quantidade', 'Fabricante'])
        for r in rows:
            ws.append([r['carimbodatahora'], r['base'], r['codigoproduto'],
                       r['descricaoproduto'], r['quantidademediavenda'], r['marca']])
