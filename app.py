...
# ============================================================
# COMPRAS DIÁRIAS — CONTROLE E ANÁLISE COMPILADA
# ============================================================
...
def compras_upload():
    try:
        ...
        # 1. Descobre qual foi a última planilha enviada para comparar
        cursor.execute("SELECT id FROM purchases_uploads ORDER BY id DESC LIMIT 1")
        ultimo_upload_row = cursor.fetchone()
        ultimo_upload_id = ultimo_upload_row[0] if ultimo_upload_row else None

        # 2. Registra o lote na tabela purchases_uploads
        cursor.execute("""
            INSERT INTO purchases_uploads (userId, fileName, totalItems)
            VALUES (1, 'Compras_Diarias.xlsx', %s) RETURNING id
        """, [len(items)])
        novo_upload_id = cursor.fetchone()[0]

        # 3. Salva todos os produtos na tabela purchase_items
        for item in items:
            cursor.execute("""
                INSERT INTO purchase_items (uploadId, codigoProduto, descricaoProduto, quantidadeMediaVenda, marca)
                VALUES (%s, %s, %s, %s, %s)
            """, [
                novo_upload_id,
        ...
        # 4. Processamento dos KPIs Analíticos
        total_planilha = len(items)
        existiam_na_anterior = 0
        novos_produtos = 0

        codigos_atuais = [str(item.get('codigoProduto', '')).strip() for item in items if item.get('codigoProduto')]

        # Descobre o que já estava na planilha anterior
        ultimos_codigos_anteriores = []
        if ultimo_upload_id:
            cursor.execute("SELECT DISTINCT codigoProduto FROM purchase_items WHERE uploadId = %s", [ultimo_upload_id])
            ultimos_codigos_anteriores = [str(row[0]).strip() for row in cursor.fetchall()]

        if ultimo_upload_id:
            existiam_na_anterior = sum(1 for cod in codigos_atuais if cod in ultimos_codigos_anteriores)
        else:
            existiam_na_anterior = 0

        # Descobre quais produtos são 100% novos no histórico do banco
        if codigos_atuais:
            placeholders = ','.join(['%s'] * len(codigos_atuais))
            query_historico = f"SELECT DISTINCT codigoProduto FROM purchase_items WHERE codigoProduto IN ({placeholders}) AND uploadId != %s"
            cursor.execute(query_historico, codigos_atuais + [novo_upload_id])
            codigos_historicos = [str(row[0]).strip() for row in cursor.fetchall()]
            novos_produtos = sum(1 for cod in codigos_atuais if cod not in codigos_historicos)
        else:
            novos_produtos = total_planilha

        conn.commit()
        cursor.close()
        conn.close()
        cache_clear() # Atualiza o cache para aparecer na hora

        return jsonify({
            'success': True,
            'uploadId': novo_upload_id,
            'analise': {
                'totalPlanilha': total_planilha,
