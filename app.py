...
def shelflife_upload():
    ...
    for p in produtos:
        ...
        cod_sl_val = str(p.get('cod_sl') or '').strip()
        is_sl = bool(cod_sl_val)  # SL somente quando cod_sl estiver preenchido
...
def shelflife_corrigir_is_sl():
    """Corrige o campo is_sl no banco: true somente quando cod_sl estiver preenchido."""
    try:
        conn   = get_conn()
        cursor = conn.cursor()
        # is_sl = true somente se cod_sl não for nulo e não for string vazia
        cursor.execute("""
            UPDATE shelflife
            SET is_sl = CASE
                WHEN cod_sl IS NOT NULL AND TRIM(cod_sl) <> '' THEN TRUE
                ELSE FALSE
            END
        """)
