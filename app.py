...
# ============================================================
#  CLIENTES EM RISCO — Sem compra há X dias
# ============================================================
@app.route('/api/clientes-em-risco')
def clientes_em_risco():
    try:
        dias  = int(request.args.get('dias', 60))
        limit = int(request.args.get('limite', 100))
        p     = montarParams(request.args)
        conn  = get_conn(); cur = conn.cursor()
        ...
        cur.execute("""
            SELECT
                ...
                CURRENT_DATE - MAX(f.data_movimento::DATE) AS dias_sem_compra,
        ...
        # Converte tipos
        for r in rows:
            r['dias_sem_compra'] = int(r['dias_sem_compra']) if r['dias_sem_compra'] else 0
            r['fat_total'] = float(r['fat_total']) if r['fat_total'] else 0
            r['num_pedidos'] = int(r['num_pedidos']) if r['num_pedidos'] else 0
...
def comparar_periodos():
    try:
        ...
        def buscar(ano, mes):
            params = []
            where  = ['tipo_operacao IN (%s, %s)']
            params += ['Venda', 'Devolucao']
            ...
            sql = """
                SELECT
                    ...
                    ROUND(AVG(CASE WHEN tipo_operacao='Venda' THEN valor_nf END)::NUMERIC,2) AS ticket_medio
            ...
            return {
                'faturamento': fat, 'devolucoes': dev,
                'liquido': fat + dev,
                'clientes': int(r[2] or 0),
                'pedidos': int(r[3] or 0),
                'ticket_medio': float(r[4] or 0),
            }

        periodo_a = buscar(ano_a, mes_a)
        periodo_b = buscar(ano_b, mes_b)
        ...
        return jsonify({
            ...
            'periodo_b': periodo_b,
            'variacao': {
                'faturamento':  variacao(periodo_a['faturamento'],  periodo_b['faturamento']),
                'devolucoes':   variacao(periodo_a['devolucoes'],   periodo_b['devolucoes']),
                'liquido':      variacao(periodo_a['liquido'],      periodo_b['liquido']),
                'clientes':     variacao(periodo_a['clientes'],     periodo_b['clientes']),
                'pedidos':      variacao(periodo_a['pedidos'],      periodo_b['pedidos']),
                'ticket_medio': variacao(periodo_a['ticket_medio'], periodo_b['ticket_medio']),
            }
        })
    except Exception as e:
        return jsonify({'erro': str(e)}), 500
