...
def compras_upload():
    """Recebe os itens da planilha (JSON) e grava no Railway. Suporta envio em lotes."""
    conn = None
    try:
        data                = request.get_json(force=True)
        items               = data.get('items', [])
        nome_arquivo        = data.get('nomeArquivo', 'Compras_Diarias.xlsx')
        upload_id_existente = data.get('uploadIdExistente', None)
        if not items:
        ...
        # Detecta base pelo nome do arquivo
        ...
        if 'CGS' in nome_upper:
            ...
            base = str(items[0].get('base', '')) if items else ''

        if upload_id_existente:
            # Lote adicional — aproveita upload já criado
            novo_upload_id   = int(upload_id_existente)
            ultimo_upload_id = None
        else:
            # Primeiro lote — cria novo registro
