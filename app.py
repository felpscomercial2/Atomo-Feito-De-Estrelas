...
def aquecer_cache_inicial():
    """Aquece o cache de filtros assim que o servidor sobe."""
    import threading
    def _warm():
        import time
        time.sleep(3)  # aguarda o servidor estar pronto
        try:
            conn   = get_conn()
            cursor = conn.cursor()
            def q(sql):
...
# Aquece o cache quando o módulo é carregado pelo gunicorn
aquecer_cache_inicial()
