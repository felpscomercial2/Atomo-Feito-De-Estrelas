...
def valter_contexto():
    try:
        ...
        # Top 10 vendedores com detalhes
        vends = q("""
            SELECT vendedor, cod_vendedor,
                ROUND(SUM(CASE WHEN tipo_operacao='Venda'     THEN valor_nf ELSE 0 END)::NUMERIC,2) AS fat,
                ROUND(SUM(CASE WHEN tipo_operacao='Devolucao' THEN valor_nf ELSE 0 END)::NUMERIC,2) AS dev,
                COUNT(DISTINCT cod_cliente) AS clientes
            FROM faturamento WHERE ano=%s AND vendedor IS NOT NULL
            GROUP BY vendedor, cod_vendedor ORDER BY fat DESC LIMIT 10""", [ano])
        ...
        marcas = q("""
            ...
            GROUP BY marca ORDER BY fat DESC LIMIT 10""", [ano])

        # Top 10 produtos
        prods = q("""
            SELECT produto, cod_produto,
                ROUND(SUM(CASE WHEN tipo_operacao='Venda' THEN valor_nf ELSE 0 END)::NUMERIC,2) AS fat,
                SUM(CASE WHEN tipo_operacao='Venda' THEN quantidade ELSE 0 END) AS qtd
            FROM faturamento WHERE ano=%s AND produto IS NOT NULL
            GROUP BY produto, cod_produto ORDER BY fat DESC LIMIT 10""", [ano])

        # Faturamento por mês (ano atual)
        meses_fat = q("""
            SELECT mes,
                ROUND(SUM(CASE WHEN tipo_operacao='Venda'     THEN valor_nf ELSE 0 END)::NUMERIC,2),
                ROUND(SUM(CASE WHEN tipo_operacao='Devolucao' THEN valor_nf ELSE 0 END)::NUMERIC,2)
        ...
        ctx = f"""SISTEMA ÁTOMO — DADOS COMPLETOS ({ano})
══════════════════════════════════════════════

FATURAMENTO {ano}:
  Total vendas:     R$ {k[0]:,.2f}
  Total devoluções: R$ {k[1]:,.2f}
  Líquido:          R$ {(k[0]-k[1]):,.2f}
  Clientes ativos:  {k[2]:,}
  Vendedores:       {k[3]}

MÊS ATUAL ({meses_nome.get(mes,'')}/{ano}):
  Vendas:     R$ {km[0]:,.2f}
  Devoluções: R$ {km[1]:,.2f}

FATURAMENTO MENSAL {ano} (Vendas | Devoluções):
{chr(10).join(f"  {meses_nome.get(r[0],r[0])}: R$ {r[1]:,.2f} | Dev: R$ {r[2]:,.2f}" for r in meses_fat)}

TOP 10 VENDEDORES {ano} (código | nome | faturamento | devoluções | clientes):
{chr(10).join(f"  {i+1}. Cód {r[1]} — {r[0]} | Fat: R$ {r[2]:,.2f} | Dev: R$ {r[3]:,.2f} | {r[4]} clientes" for i,r in enumerate(vends))}

TOP 10 MARCAS {ano} (marca | faturamento | clientes):
{chr(10).join(f"  {i+1}. {r[0]} | R$ {r[1]:,.2f} | {r[2]} clientes" for i,r in enumerate(marcas))}

TOP 10 PRODUTOS {ano} (produto | código | faturamento | quantidade):
{chr(10).join(f"  {i+1}. {r[0]} (cod:{r[1]}) | R$ {r[2]:,.2f} | {int(r[3] or 0):,} un" for i,r in enumerate(prods))}

SHELF LIFE (semana atual):
  Total: {sl[0]} produtos | Críticos (≤30d): {sl[1]} | Em ação SL: {sl[2]} | Sem giro: {sl[3]}

CARTEIRA DE CLIENTES:
  Total: {cart[0]:,} clientes | {cart[1]} vendedores
"""
...
def valter_alertas():
    try:
        ...
        cur.execute("""SELECT COUNT(*) FROM shelflife
            WHERE semana=(SELECT MAX(semana) FROM shelflife)
            AND dias_vencimento<=7 AND dias_vencimento>=0 AND is_sl=TRUE""")
        sl_urgente = cur.fetchone()[0]
        if sl_urgente > 0:
            alertas.append({'icone':'🚨',
        ...
        crit = cur.fetchone()[0]
        if crit > 0:
            alertas.append({'icone':'⚠️',
                'texto': f'{crit} produto{"s" if crit>1 else ""} crítico{"s" if crit>1 else ""} sem ação definida',
                'pergunta': f'Quais são os {crit} produtos críticos sem ação? Sugira o que fazer.'})
