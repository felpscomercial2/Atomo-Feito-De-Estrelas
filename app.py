...
# ------------------------------------------------------------
# Cache HTTP: o navegador reaproveita a resposta por 5 min e
# ainda pode usar a versao antiga por 24h enquanto revalida.
# So para GET de leitura (nao afeta upload/POST/DELETE).
# ------------------------------------------------------------
...
def shelflife_manual():
    ...
    semana      = data.get('semana')
    unidade     = data.get('unidade') or 'PR'
    cod_produto = str(data.get('cod_produto') or '').strip()
    produto     = str(data.get('produto') or '').strip()

    if not semana or not cod_produto or not produto:
        return jsonify({'erro': 'Campos obrigatorios ausentes (semana, cod_produto, produto)'}), 400

    validade = data.get('validade', '')
    val_fmt  = str(validade)[:10] if validade and len(str(validade)) >= 10 else None

    hoje = _date.today()
    try:
        from datetime import datetime
        val_date = datetime.strptime(val_fmt, '%Y-%m-%d').date()
        dias     = (val_date - hoje).days
    except Exception:
        dias = 999
    ...
    try:
        cursor.execute("""
            INSERT INTO shelflife (
                semana, unidade, cod_produto, cod_sl, produto, marca,
                quantidade_log, validade, dias_vencimento,
                status_logistica, status, is_sl, valor_sl,
                quantidade_atual, venda_3meses, venda_mes, obs_gerais
            ...
            DO UPDATE SET
                ...
                status           = EXCLUDED.status,
                is_sl            = EXCLUDED.is_sl,
                valor_sl         = COALESCE(EXCLUDED.valor_sl, shelflife.valor_sl),
                quantidade_atual = COALESCE(EXCLUDED.quantidade_atual, shelflife.quantidade_atual),
                venda_3meses     = COALESCE(EXCLUDED.venda_3meses, shelflife.venda_3meses),
                venda_mes        = COALESCE(EXCLUDED.venda_mes, shelflife.venda_mes),
                obs_gerais       = COALESCE(EXCLUDED.obs_gerais, shelflife.obs_gerais),
                updated_at       = NOW()
            RETURNING id
        """, [
            semana, unidade, cod_produto, data.get('cod_sl'), produto, data.get('marca'),
            data.get('qtde_log') or 0, val_fmt, dias,
            'MANUAL', status, is_sl, data.get('valor_sl'),
            data.get('qtde_atual'), data.get('venda_3m'), data.get('venda_mensal'),
            data.get('obs_gerais')
        ])
...
def pivot_clientes_novo():
    """
    Cruza FATURAMENTO x CARTEIRA com chave HIBRIDA (codigo do cliente + base + CNPJ).

    Prioridade de match:
      1) codigo + unidade (base) iguais -> match perfeito
      2) CNPJ igual                     -> mesmo cliente com codigo diferente
    Em empate, ganha a carteira do vendedor filtrado.
    O codigo exibido e SEMPRE o da carteira quando ha match (evita codigos
    divergentes e cliente duplicado); o codigo da nota vai em cod_cliente_nota.
    ...
    try:
        ...
        # ---------- descobre colunas em cada tabela ----------
        ...
        # tipo_operacao normalizado (sem acento / maiusculo) -> 'VENDA' | 'DEVOLUCAO'
        ...
        # cart_cod : 1 linha por (codigo de cliente + base)  -> o mesmo codigo em
        #            bases diferentes e cliente diferente
        ...
        vend_params = []
        if cods:
            # Vendedor = dono do cliente na CARTEIRA.
            # Venda de cliente SEM carteira fica com o vendedor da propria nota,
            # para o total continuar batendo com a planilha.
        ...
        if produtos:
            ...
            # clientes da carteira que nao compraram no periodo
            cart_zero   = ''
            zero_params = []
            if cods or unidades:
                where_zero = []
...
def clientes_produtos_export():
    try:
        ...
        if periodos:
            pares = ["%s-%s" % (int(p.get('ano')), int(p.get('mes'))) for p in periodos if p.get('ano') and p.get('mes')]
            if pares:
