...
def shelflife_atualizar():
    ...
    cursor.execute(f"""
        UPDATE shelflife SET
            ...
            is_sl                = CASE WHEN %s IS NOT NULL THEN %s ELSE is_sl END,  -- recalculado abaixo
            updated_at           = NOW()
        WHERE id = %s
    """, [
        ...
        *_data_inc_params,
        ...
        data.get('cod_sl'),  (lambda _c=str(data.get('cod_sl') or '').strip(): bool(_c) and _c not in ('-', '—', '--', 'nan', 'none', 'null', '0'))(),
        id_prod
    ])
