...
# ============================================================
def _aquecer_cache():
    """Aquece todos os caches pesados em background."""
    import threading
    def _warm():
        try:
            conn   = get_conn()
            ...
            # Todos os produtos
            cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
            cursor.execute("SELECT DISTINCT produto, cod_produto, marca FROM faturamento WHERE produto IS NOT NULL AND produto != '' ORDER BY produto")
            produtos = [_serializar_row(dict(r)) for r in cursor.fetchall()]
...
def filtros():
    ...
    cursor.close(); release_conn(conn)
...
def shelflife_migrar():
    ...
    conn.commit(); cursor.close(); release_conn(conn)
    ...
    try:
        ...
        if produtos:
            ...
            sql = (
                ...
                ' FROM carteira c'
                ' LEFT JOIN (SELECT cod_vendedor, vendedor FROM (SELECT cod_vendedor, vendedor, ROW_NUMBER() OVER (PARTITION BY cod_vendedor ORDER BY COUNT(*) DESC) as rn FROM faturamento WHERE vendedor IS NOT NULL AND vendedor != %s GROUP BY cod_vendedor, vendedor) sub WHERE rn = 1) v'
